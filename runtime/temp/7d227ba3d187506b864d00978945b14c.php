...
layui.use('table', function(){
  ...
  table.render({
    elem: '#demo'
    ,height: 312
    ,url: "<?php echo url('Cfi/buyCfi2'); ?>" //数据接口
  ...
  console.log(obj.data.id) //得到当前行数据id
            $.ajax({
                url:"<?php echo url('cfi/dealDetail'); ?>",
                data:{
                    id:obj.data.id,
                },
                dataType:'json',
                ...
                success:function (data) {
                  alert(data);
                  window.location.href = "<?php echo url('cfi/dealDetail'); ?>";
                }
            })
            return false;
});
