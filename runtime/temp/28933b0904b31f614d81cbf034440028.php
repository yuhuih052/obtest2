<?php if (!defined('THINK_PATH')) exit(); /*a:1:{s:73:"D:\phpstudy_pro\WWW\obtest\public/../app/admin\view\widget\file\file.html";i:1585716400;}*/ ?>
...
<div class="upload-img-box<?php echo $widget_data['name']; ?>">
    ...
    <div class="upload-pre-file">
        <div style="cursor:pointer;" class="file_del"  onclick="fileDel<?php echo $widget_data['name']; ?>(this)" ><img src="/static/widget/admin/file/uploadify-cancel.png" /></div> 
...
<script type="text/javascript">
    ...
    function uploadFile<?php echo $widget_data['name']; ?>(file, data){
        ...
        $(".upload-img-box<?php echo $widget_data['name']; ?>").html('<div class="upload-pre-file">    <div style="cursor:pointer;" class="file_del"  onclick="fileDel<?php echo $widget_data['name']; ?>(this)" ><img src="/static/widget/admin/file/uploadify-cancel.png" /></div>      <span class="upload_icon_all"></span><a target="_blank" href="'+src+'"> ' + src + ' <a></div>');
    }
    ...
    function changeFile<?php echo $widget_data['name']; ?>(file,fileCount,next) {
        ...
        reader.onload = function (ev) {
            ...
            $.post("<?php echo url('File/checkFileExists'); ?>",{sha1:sha1}, function (res) {
                if(res.code) {
                    uploadFile<?php echo $widget_data['name']; ?>(file,JSON.stringify(res.data));
                }else {
                    //不存在图片则调用下一步
                    next(file,fileCount);
                }
