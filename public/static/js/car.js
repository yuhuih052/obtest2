...
function addArray(id,num){
    ...
    if(isnew==1){
		...
		productName = $('#list_'+id).attr('dataname');
		productPrice = parseFloat($('#list_'+id).attr('dataprice'));
		productId = $('#list_'+id).attr('dataid');
...
function buyNow(url,buy){
	...
	//保存到cookie1天
	$.cookie("mini_car", JSON.stringify(carArray),{expires:7, path: "/" }); 
	location.href = url;
...
function getCar() {
	if($.cookie("car")) {
		return JSON.parse($.cookie("car"));
	}
	return [];
};

function setCar(car) {
	$.cookie("car", JSON.stringify(car),{expires:7, path: "/" }); 
};

function getCarNum() {
	var car = getCar();
	var num = 0;
	mui.each(car, function(i, n) {
		num += parseInt(n.num);
	});
	return num;
};

function getCarGoods(id) {
	var car = getCar();
	var goodsItem = {
		id: id,
		num: 0
...
function addCarGoods(id, num) {
	var car = getCar();
	var hadItem;
	mui.each(car, function(i, n) {
		if(n.id == id) {
			hadItem = true;
			n.num = parseInt(n.num) + parseInt(num);
			return false;
		}
	});
	if(!hadItem) {
...
function removeCarGoods(idObj) {
	var car = getCar();
	if(Array.isArray(idObj)) {
		mui.each(idObj, function(idI, id) {
			mui.each(car, function(i, n) {
				if(n.id == id) {
					car.splice(i, 1);
					return false;
