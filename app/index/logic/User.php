 ...
 /************** 直推奖励开始**/
                ...
                if($D_value >= $zhitui_bonus){
                    ...
                    $re_zhi = [
                        ...
                        'baoguanjin'=>$p_zhi[0]['baoguanjin'] + $zhitui_bonus*0.1,
                        'all_bonus' =>  $p_zhi[0]['all_bonus'] + $zhitui_bonus,
                    ];
                    //直推人获奖更新
                    ...
                    $bill1 = [
                        ...
                        'baoguanjin'  => "+".$zhitui_bonus*0.1,
                        'bonus'   => "+".$zhitui_bonus*0.9,
                        'tuijian'   => "+".$zhitui_bonus,
                        'shuoming' => "激活会员"
                    ];
...
/************** 直推奖励结束**/
                //增加业绩
                $this->addYeji($p[0]['p_path_id'],$p[0]['id'],$check_dis['baodanbi_co'],$p,$check_dis);
                //见点奖发放
                $this->jiandianjiang($p[0]['p_path_id'],$p,$check_dis);
/**************************见点奖发放结束****/
    ...
    //见点奖发放
    public function jiandianjiang($mm,$p,$check_dis){
                //见点奖发放
                //获取被激活者推荐人的绝对路径id
                //dd($mm);
                $p_id = explode(',', $mm);
                ...
                for($last_id = count($p_id)-1; $last_id>=0 && $last_id>$last_id-25;$last_id--){
                    ...
                    //应获奖
                    //奖金 = 被激活者等级信息 * 激活者等级相应百分比
                    $p_id_rank_bonus =  $check_dis['baodanbi_co'] * $p_id_rank['jiandian_co'];
                    //当日累计奖金
    ...
    public function upgrade($data){
        ...
        if($member[0]['wallet'] >= $bdb_del){
            ...
            //报单中心扣除报单币记录
                ...
                $this->modelBill->setInfo($bill12);
                //dd('1');
             $this->addYeji($member[0]['p_path_id'],$member[0]['id'],$bdb_del,$member,$new_v);
        /****对碰奖结算，结束******/
    ...
    //统计业绩
    public function addYeji($member_path,$member_id,$bdb_del,$p,$check_dis){
            //被激活的会员的产生的报单币 -> $bdb_del
            //左右业绩统计
            //增加总业绩
            //获取接点人id
        ...
        /***左右区业绩统计，结束**/
            ...
            foreach (array_reverse($hello_id) as $key => $value) {
                ...
                if($dp_info[0]['LiftChild'] && $dp_info[0]['RightChild']){
                  ...
                  //应获奖额 = 被激活会员的投资（产生对碰额度） * 获奖会员等级比例 ； 
                  //如果被激活者是1星会员，那么获奖是： 奖金 = 100 * 5%；
                  $dp_info_rank_bonus = $a * $dp_info_rank['duipeng_co'];
                    ...
                    //发放管理奖
                    //获奖额度，奖金 = 被激活者等级对应金额 * 激活者等级对应百分比；
                    $first_father_rank_bonus = $bdb_del * $first_father_rank['guanli_co'];
                    //日封顶与当日获奖差值
                    ...
                    for($i = 1; count($dp_p_id_path)>$i && $i<=2; $i++){
                        ...
                        if($f_father_info[0]['member_rank']>1){
                            ...
                            //应获奖金
                            //奖金 = 被激活者等级投资金额 * 获奖者等级相应奖金百分比
                            $f_father_rank_bonus = $bdb_del * $f_father_rank['guanli_co'];
                            //差值
...
///////////////////////////////////////////////////////////////////////////////////
    ...
    public function bonusDetail(){
        $id = session('user_id2');
        return $this->modelBonusDetail->where('user_id','in',$id)->select();
    }
