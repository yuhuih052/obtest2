						...
						if($now_new_price < 4){
							...
							if($tt_amount > $user_tt_amount){//需求量大于账户剩余购买量
								...
								if($over_tt_pay > $in_now_pay){ 
									...
									$a = floor($over_tt_pay / 2);

									$this->modelMember->where('id',$id)->inc('CFI',$rise_capacity+15000+$a)
																		->dec('dianzibi',$now_pay+15000*3.9+$over_tt_pay)
																		->update();
									$this->bill($id,$member['username'],'dianzibi_all',$now_pay+15000*3.9+$over_tt_pay);
									return [RESULT_SUCCESS,'购买成功'];
								}
